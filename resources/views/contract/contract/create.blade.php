            ...
            <div class="modal-content">
                ...
                <div class="modal-body bg-custom">
                    ...
                    <form id="customerForm" action="{{ route('customer.store') }}" method="POST">
                        @csrf
                        <div class="mb-3">
                            <label for="customer_full_name" class="form-label">Full Name</label>
                        ...
                        <div class="mb-3">
                            <label for="customer_card_number" class="form-label">Card Number</label>
                            <input type="text" class="form-control" id="customer_card_number"
                                name="customer_card_number" required>
                        </div>
                        <div class="mb-3">
                            <label for="customer_card_issud_auth" class="form-label">Card Issued Authority</label>
                            <input type="text" class="form-control" id="customer_card_issud_auth"
                                name="customer_card_issud_auth" required>
                        </div>
                        <div class="mb-3">
                            <label for="customer_card_issud_date" class="form-label">Card Issued Date</label>
                            <input type="date" class="form-control" id="customer_card_issud_date"
                                name="customer_card_issud_date" required>
                        </div>
                        <!-- Hidden fields for user_create and user_update -->
                        <input type="hidden" name="user_id_create" value="{{ auth()->user()->id }}">
                        <input type="hidden" name="user_id_update" value="{{ auth()->user()->id }}">
                    </form>
                </div>
                <div class="modal-footer">
                    <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Close</button>
                    <button type="submit" form="customerForm" class="btn btn-primary" id="saveCustomerBtn">Save
                        changes</button>
                </div>
            </div>
        </div>
            ...
            $('#customerForm').on('submit', function(e) {
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        // Optionally, refresh the DataTable or update the UI with the new customer
                    },
                    error: function(xhr) {
